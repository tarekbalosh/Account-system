  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      saleSummary?.data?.forEach((item: any) => {
        ...
        mergedMap.set(item.period, { name, income: item.total, expenses: 0 });
      });
      ...
      <motion.div variants={itemVars} className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={fetchData}
            disabled={isLoading}
            className="p-3.5 rounded-2xl bg-slate-900 border border-white/5 text-slate-400 hover:text-blue-500 hover:border-blue-500/30 transition-all active:scale-95 disabled:opacity-50"
          >
        ...
        {statCards.map((card, i) => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-black tracking-widest border shrink-0 ${card.isPositive
        ...
        <motion.div variants={itemVars} className="lg:col-span-4 flex flex-col gap-6">
          <div className="flex-1 bg-slate-900 border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              ...
              <Link href="/expenses" className="group/btn flex items-center gap-1.5 text-blue-500 text-[10px] font-black uppercase tracking-widest hover:text-blue-400 transition-colors">
            ...
            <div className="space-y-4 relative z-10">
              {recentTransactions.map((trx, i) => (
                <div key={i} className="flex items-center gap-4 p-3.5 rounded-[1.5rem] bg-slate-950/40 border border-white/5 hover:border-white/10 transition-colors group/item">
                  <div className={`p-2.5 rounded-xl border border-white/10 ${trx.type === 'INCOME' ? 'bg-blue-500/10 text-blue-500' : 'bg-rose-500/10 text-rose-500'
                    }`}>
                  ...
                  <div className="text-right">
                    <p className={`text-sm font-black tabular-nums ${trx.type === 'INCOME' ? 'text-blue-500' : 'text-rose-500'
                      }`}>
